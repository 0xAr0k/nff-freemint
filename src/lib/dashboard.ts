  ...
  <script>
    ...
    function renderSubmissions(subs) {
      ...
      for (var i = 0; i < subs.length; i++) {
        var s = subs[i];
        var curiosityClean = String(s.curiosity || '-').split('\\n').join(' ').trim();
        h += '<tr>';
        h += '<td>' + escapeHtml(formatDate(s.timestamp)) + '</td>';
        h += '<td>' + escapeHtml(s.usernameOriginal || s.username) + '</td>';
        ...
        h += '<td class="truncate" title="' + escapeHtml(curiosityClean) + '">' + escapeHtml(curiosityClean) + '</td>';
    ...
    function exportCSV() {
      showAlert('Fetching all data for export...', 'success');
      ...
      fetchAllPages(1, []).then(function(allData) {
        ...
        for (var i = 0; i < allData.length; i++) {
          ...
          csv += [
            s.timestamp || '',
            '"' + (s.usernameOriginal || s.username || '').replace(/"/g, '""') + '"',
            '"' + (s.discordId || '').replace(/"/g, '""') + '"',
            s.ethAddress || '',
            '"' + (s.curiosity || '').replace(/"/g, '""').replace(/\\n/g, ' ') + '"',
    ...
    function fetchAllPages(page, accumulated) {
      return fetch('/admin/records?page=' + page + '&limit=100')
        ...
        .then(function(data) {
          ...
          if (d.pagination && d.pagination.hasMore) {
            return fetchAllPages(page + 1, all);
          }
          return all;
        });
    }
